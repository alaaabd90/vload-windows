...
namespace Vload_sub {
    ...
    void RawUpdater_FixEnt(const std::shared_ptr<Vload::ProxyEntity> &ent) {
        if (ent == nullptr) return;
        auto stream = Vload_fmt::GetStreamSettings(ent->bean.get());
    ...
    void RawUpdater::update(const QString &str) {
        ...
        // Vload format
        if (str.startsWith("vload://")) {
            needFix = false;
            auto link = QUrl(str);
            if (!link.isValid()) return;
            ent = Vload::ProfileManager::NewProxyEntity(link.host());
        ...
        if (str.startsWith("socks5://") || str.startsWith("socks4://") ||
            ...
            ent = Vload::ProfileManager::NewProxyEntity("socks");
        ...
        if (str.startsWith("http://") || str.startsWith("https://")) {
            ent = Vload::ProfileManager::NewProxyEntity("http");
            auto ok = ent->SocksHTTPBean()->TryParseLink(str);
            if (!ok) return;
        }
        ...
        if (str.startsWith("ss://")) {
            ent = Vload::ProfileManager::NewProxyEntity("shadowsocks");
        ...
        if (str.startsWith("vmess://")) {
            ent = Vload::ProfileManager::NewProxyEntity("vmess");
        ...
        if (str.startsWith("vless://")) {
            ent = Vload::ProfileManager::NewProxyEntity("vless");
        ...
        if (str.startsWith("trojan://")) {
            ent = Vload::ProfileManager::NewProxyEntity("trojan");
            auto ok = ent->TrojanVLESSBean()->TryParseLink(str);
            if (!ok) return;
        }
        ...
        if (str.startsWith("naive+")) {
            needFix = false;
            ent = Vload::ProfileManager::NewProxyEntity("naive");
        ...
        if (str.startsWith("hysteria2://") || str.startsWith("hy2://")) {
            needFix = false;
            ent = Vload::ProfileManager::NewProxyEntity("hysteria2");
        ...
        if (str.startsWith("tuic://")) {
            needFix = false;
            ent = Vload::ProfileManager::NewProxyEntity("tuic");
            auto ok = ent->QUICBean()->TryParseLink(str);
            if (!ok) return;
        }
        ...
        // End
        Vload::profileManager->AddProfile(ent, gid_add_to);
        updated_order += ent;
    }
    ...
    void RawUpdater::updateClash(const QString &str) {
        ...
        try {
            ...
            for (auto proxy: proxies) {
                ...
                auto ent = Vload::ProfileManager::NewProxyEntity(type);
                ...
                if (type_clash == "ss") {
                    ...
                    if (bean->stream->utlsFingerprint.isEmpty()) {
                        bean->stream->utlsFingerprint = Vload::dataStore->utlsFingerprint;
                    }
    ...
    void GroupUpdater::Update(const QString &_str, int _sub_gid, bool _not_sub_as_url) {
        // 创建 rawUpdater
        Vload::dataStore->imported_count = 0;
        ...
        auto group = Vload::profileManager->GetGroup(_sub_gid);
        ...
        QList<std::shared_ptr<Vload::ProxyEntity>> in;          // 更新前
        QList<std::shared_ptr<Vload::ProxyEntity>> out_all;     // 更新前 + 更新后
        QList<std::shared_ptr<Vload::ProxyEntity>> out;         // 更新后
        QList<std::shared_ptr<Vload::ProxyEntity>> only_in;     // 只在更新前有的
        QList<std::shared_ptr<Vload::ProxyEntity>> only_out;    // 只在更新后有的
        QList<std::shared_ptr<Vload::ProxyEntity>> update_del;  // 更新前后都有的，需要删除的新配置
        QList<std::shared_ptr<Vload::ProxyEntity>> update_keep; // 更新前后都有的，被保留的旧配置
        ...
        if (group != nullptr) {
            ...
            group->Save();
            //
            if (Vload::dataStore->sub_clear) {
                MW_show_log(QObject::tr("Clearing servers..."));
                for (const auto &profile: in) {
                    Vload::profileManager->DeleteProfile(profile->id);
                }
            }
        }
        ...
        if (group != nullptr) {
            ...
            if (Vload::dataStore->sub_clear) {
                ...
                Vload::ProfileFilter::OnlyInSrc_ByPointer(out_all, in, out);
                Vload::ProfileFilter::OnlyInSrc(in, out, only_in);
                Vload::ProfileFilter::OnlyInSrc(out, in, only_out);
                Vload::ProfileFilter::Common(in, out, update_keep, update_del, false);
            ...
            Vload::dataStore->imported_count = rawUpdater->updated_order.count();
...
void serialUpdateSubscription(const QList<int> &groupsTabOrder, int _order, bool onlyAllowed) {
    ...
    auto group = Vload::profileManager->GetGroup(groupsTabOrder[_order]);
    ...
    while (nextOrder < groupsTabOrder.size()) {
        ...
        auto nextGroup = Vload::profileManager->GetGroup(nextGid);
    ...
    Vload_sub::groupUpdater->AsyncUpdate(group->url, group->id, [=] {
...
void UI_update_all_groups(bool onlyAllowed) {
    ...
    auto groupsTabOrder = Vload::profileManager->groupsTabOrder;
