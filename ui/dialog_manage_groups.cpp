...
#define AddGroupToListIfExist(_id)                       \
    auto __ent = Vload::profileManager->GetGroup(_id); \
    if (__ent != nullptr) {                              \
...
DialogManageGroups::DialogManageGroups(QWidget *parent) : QDialog(parent), ui(new Ui::DialogManageGroups) {
    ...
    for (auto id: Vload::profileManager->groupsTabOrder) {
...
void DialogManageGroups::on_add_clicked() {
    auto ent = Vload::ProfileManager::NewGroup();
    auto dialog = new DialogEditGroup(ent, this);
    int ret = dialog->exec();
    dialog->deleteLater();

    if (ret == QDialog::Accepted) {
        Vload::profileManager->AddGroup(ent);
